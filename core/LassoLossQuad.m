function [yp, errVal, errGradVec] = LassoLossQuad(theta, x, y, varargin)
%
% [yp, errVal, errGradVec] = LassoLossQuad(theta, x, y, OPTIONS)
%
% LassoLossQuad computes the forward prediction using the current set of parameters,
...
errGradVec = (-1/numberOfPoints) .* [sum(sign(y - yp).*(x.^2)); sum(sign(y - yp).*(x)); sum(sign(y - yp))];
